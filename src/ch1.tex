...
\dfn{样本空间}{
    考虑样本空间集合$S$, 我们有$S\defeq\{\mbox{所有样本点}\}$.
}
由定义, 我们可以得到几种特殊的样本空间:
\begin{itemize}
    \item $\varnothing$事件: 不可能发生的事件.
    \item $S-\varnothing$发生的事件.
    \item 基本事件$\omega$: $|\omega|=1$ \ie 基本事件只含有一个样本点.
\end{itemize}
...
\begin{align}
    \begin{split}
        {\bigcup_{i=1}^{n }A_{i}}\; \mathrm{happen} & \iff \exists i \in [1, n] \; \st\; A_i \; \mathrm{happens} \\
        {\bigcap_{i=1}^{n}A_{i}}\; \mathrm{happen}  & \iff\forall i \in [1, n]\; \st\; A_i\; \mathrm{happens}
    \end{split}
\end{align}
\section{频率}
\dfn{频率}{
...
\section{概率}
\dfn{概率}{
    \emph{(Kolmogorov公理化定义)}设有随机试验$E$且与之对应的样本空间$S$, 考虑事件$A$\\
...
\thm{}{
    设$A_1, A_2, \ldots, A_n$是$n$个互斥事件, 则有
    \begin{equation}
...
\ex{``彩票''}{
    第一次买中的概率为$\dfrac{1}{2}$, 第二次买中而第一次未中的概率是$\dfrac{7}{10}$, 第三次买中而前两次未中的概率是$\dfrac{9}{10}$, 求三次都未中的概率.
}
\sol{
    以$A_i(i=1, 2, 3)$表示事件``第$i$次买中'', 以B表示事件``三次都未中'', 由乘法公式
    \leftnote{\eqref{eq:mul}}
    \begin{align*}
...
\subsection{全概率公式}
\mlemma{完备事件组}{
    设$A_1, A_2, \ldots, A_n$是有限或可数个事件, 若其满足
    \begin{enumerate*}
        \item 两两互斥, \hspace{4pt}
        \item $\bigcup_{i=1}^{n}A_i=S$,
    \end{enumerate*}\;
    则称$A_1, A_2, \ldots, A_n$是一个\vocab{完备事件组}.
}

\begin{wrapfigure}[4]{l}{0.4\textwidth}
    \begin{center}
        \includegraphics[scale=.6]{total_prob}
    \end{center}
    \caption{全概公式示意}
\end{wrapfigure}
~\eqref{eq:1.7}指出在复杂情况下不易计算$\Pr{B}$时应该根据具体情况构造完备事件组$\{A_i\}$, 使得事件$B$发生的概率是各事件$A_i$发生的条件下造成事件$B$发生概率的和.
\thm{全概公式}{
    设$A_1, A_2, \ldots, A_n$是一个完备事件组, 且\\$\forall i(i \in [1, n]\to\Pr{A_i} >0)$, 则对任一事件$B$, 有
    \begin{equation}
...
\pf{法一}{
    ...
    \end{enumerate}
    那么有
    \(\Pr{A} = \dfrac{\binom{4}{1}\cdot 3}{9^3}\)\, \, %
    \(\Pr{B} = \dfrac{\binom{4}{1}\cdot 3+\tbinom{4}{2}\mathrm{A_3^3}}{9^3}\)\, \, %
    \(\Pr{C} = \dfrac{\binom{4}{1} \binom{4}{1} \mathrm{A_3^3}}{9^3}\)%
    相加得\(\dfrac{156}{729}\).
}
\pf{法二}{
    对立事件\\
...
\pf{法三}{
    ...
    \begin{enumerate}[label=\bfseries\protect\circled{\arabic*}]
        \item A = \{三个数中有两个相同的\}
        \item B = \{三个数全不同\}
              \leftnote{\color{red}\textbf{此项须要分成下述两项, 否则会重}}
              \begin{enumerate}
    ...
    \end{enumerate}
    那么有
    \(\Pr{A} = \dfrac{\binom{4}{1}\binom{3}{1}+ \binom{4}{1}\binom{3}{1}}{9^3}\)\, \, %
    \(\Pr{B_1} = \dfrac{\binom{4}{1}\binom{4}{1}\binom{3}{1}}{9^3}\)\, \, %
    \(\Pr{B_2} = \dfrac{\binom{4}{1}\binom{4}{1}A_3^3}{9^3}\)%
    相加得\(\dfrac{156}{729}\).
}
\nt{
...
\subsection{Beyes公式}
\vocab{全概公式}是通过计算某一事件会发生的\uwave{所有原因和情况的可能性大小}来计算该事件发生的概率, 而\vocab{Beyes}公式则与之相反, 考察一件已经发生的事情的\uwave{各种原因或或情况的可能性大小}.
\thm{Beyes公式}{
    设$A_1, A_2, \ldots, A_n$是一个完备事件组, 且$\forall i (i \in [1, n]\to\Pr{A_i} >0)$, 则对任一可能发生的事件$B$, 有
    \begin{equation}
...
设$A_1, A_2, \ldots, A_n$是$n$个事件, 若对于其中任意$k(2\leq k\leq n)$个事件$A_{i_1}, A_{i_2}, \ldots, A_{i_k}$, 有
\begin{equation}
    \Pr{A_{i_1}A_{i_2}\cdots A_{i_k}}=\Pr{A_{i_1}}\Pr{A_{i_2}}\cdots\Pr{A_{i_k}}
\end{equation}
则称$A_1, A_2, \ldots, A_n$\vocab{相互独立}.
\dfn{}{
    设$A_1, A_2, \ldots, A_n\, (n > 2)$是$n$个事件, 若其中任意两个事件相互独立, 则称这$n$个事件\vocab{两两独立}.
}
由此可以得到多个独立事件所具备的性质
\cor{}{
    设$A_1, A_2, \ldots, A_n\, (n > 2)$相互独立, 则其中任意$k(2\leq k\leq n)$个(它们的对立)事件也相互独立.
}
\section{Bernoulli试验}
即两点分布.
