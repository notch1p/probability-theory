...
\sol{
    随机变量$X$的分布律为
    \begin{equation*}
        \Pr{X=k} = \frac{\lambda^k}{k!}e^{-\lambda},\,k=0,1,\cdots;\;\lambda > 0.
    \end{equation*}
    \begin{align*}
        \text{由定义得\;}
...
\end{enumerate}
第六个性质可以推广到$n$维,即若$X_1,X_2,\cdots,X_n$相互独立,则
\begin{equation*}
...
\sol{
    ...
    \begin{align*}
        ...
        \intertext{由定理(3.22), 因为$X,Y$独立,则$X^2,Y^2$也独立.有}
...
\thm{}{
    若随机变量序列$\{X_n\}\sim N$,且相互独立,则其和常数的线性组合仍然服从正态分布.
    \begin{equation}
        \ie\;\;\bmA\begin{pmatrix}
            X_1 \\ X_2 \\ \vdots \\ X_n
...
\ex{P\textsubscript{91}4}{
    若$X_i\sim N(\mu_i,\sigma_i^2),\,i=1,2,\cdots,n$,且相互独立,求$Y=\sum_{i=1}^{n}(a_iX_i + b_i)$服从什么分布.
}
\sol{
    由题意,根据定理(4.21),$Y$是$X$的一个线性组合,所以$Y$服从正态分布.
    \begin{align*}
...
\pf{证明}{
    \begin{align*}
        ...
        \end{split}
        \intertext{放缩,取$k = \dfrac{\abs{x-\mu}^2}{\epsilon^2}$.(因为$X$都落在领域外,则$k > 1$.)}
        \begin{split}
...
\subsection{大数定律}
\mlemma{随机变量序列相互独立}{
    若对于任意$n>1$,$X_1,X_2,\cdots,X_n$都相互独立,则称$X_1,X_2,\cdots,X_n,\cdots$是相互独立的.
}
\thm{大数定律}{
    设随机变量$X_1,X_2,\cdots,X_n,\cdots$相互独立,且具有相同的期望和方差
    \[\EE{X_i} = \mu,\,\Var{X_i} = \sigma^2,\,i=1,2,\cdots.\]
...
当$n$很大时,$Y_n$集中于$\mu$.
像这样表现的收敛性成为随机变量序列$Y_1,Y_2,\cdots,Y_n,\cdots$\vocab{依概率收敛}于$\mu$,记为
\begin{equation}
